...
function FFmpeg(options) {
    ...
    self._construct_options = function(){
      ...
      var last_parm = options[options.length-1].split(" ");
      var output = last_parm.pop();
      options[options.length-1] = options[options.length-1].substr(0, options[options.length-1].length - output.length-1);
    ...
    self.start = function(callback) {
        var proc = spawn('ffmpeg', self._construct_options());
