...
RedisSocket.prototype.broadcast = Promise.coroutine(function* (key, message) {
  log.debug(`RedisSocket.broadcast key: ${key}, message: ${JSON.stringify(message)} caller: ${this.constructor.name}`);
...
RedisSocket.prototype.ping = Promise.coroutine(function* (id, key, message) {
  log.debug(`RedisSocket.ping id: ${id}, key: ${key}, message: ${JSON.stringify(message)} caller: ${this.constructor.name}`);
