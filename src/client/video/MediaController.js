...
var setupMedia = Promise.coroutine(function* (domElements) {
  ...
  var mediaPromises = [video.setup(domElements.videoElement,
                                   domElements.window,
                                   domElements.mediaSource)];
  ...
  if(active.activeMeta.get(metaManager.Enums.TYPES.VIDEO)) {
    ...
    mediaPromises.push(buffers[metaManager.Enums.TYPES.VIDEO]
      .setup(metaManager.Enums.TYPES.VIDEO, domElements.mediaSource, video));

    buffers[metaManager.Enums.TYPES.VIDEO].once("init", function()
      { video.emit("get-segment", metaManager.Enums.TYPES.VIDEO, 0) });
  ...
  if(active.activeMeta.get(metaManager.Enums.TYPES.AUDIO)) {
    ...
    mediaPromises.push(buffers[metaManager.Enums.TYPES.AUDIO]
      .setup(metaManager.Enums.TYPES.AUDIO, domElements.mediaSource, video));

    buffers[metaManager.Enums.TYPES.AUDIO].once("init", function()
      { video.emit("get-segment", metaManager.Enums.TYPES.AUDIO, 0) });
  ...
  subtitles.once("subtitle-loaded", function()
    { this.emit("subtitle-loaded"); }.bind(this));
    ...
    var canEndStream = function() {
      ...
      if(!isUpdating) {
        if(mediaSource.readyState === "open") {
          ...
          if(active.activeMeta.get(metaManager.Enums.TYPES.VIDEO)) {
            buffers[metaManager.Enums.TYPES.VIDEO]
              .removeListener("error", video.resetVideoElementErrorState);
          }

          if(active.activeMeta.get(metaManager.Enums.TYPES.AUDIO)) {
            buffers[metaManager.Enums.TYPES.AUDIO]
              .removeListener("error", video.resetVideoElementErrorState);
          }
